...
import { 
  Menu, X, Phone, Mail, MapPin, Heart, Calendar, Users, Award, 
  BookOpen, User, Languages, CheckCircle, Smartphone, Camera, 
  Facebook, ChevronRight, ArrowLeft, Grid, Search, XCircle, Home, Youtube
} from 'lucide-react';
...
const TRUST_INFO = {
  ...
  youtube_url: "https://www.youtube.com/@NaaNesthamTrust" // Update with actual handle if different
};

const HEADS = [
  { name_en: "Katla Satyanarayana", name_te: "కట్ల సత్యనారాయణ", role_en: "Chairman", role_te: "చైర్మన్", img: "/images/chairman.jpg" },
  { name_en: "B.V. Purnachander", name_te: "బి.వి. పూర్ణచందర్", role_en: "Honorary Chairman", role_te: "గౌరవ చైర్మన్", img: "/images/hon_chairman.jpg" },
  { name_en: "Penta Srinivas", name_te: "పెంట శ్రీనివాస్", role_en: "Finance Trustee", role_te: "ఫైనాన్స్ ట్రస్టీ", img: "/images/finance_trustee.jpg" },
  { name_en: "Vengaldas Ashok", name_te: "వెంగళదాస్ అశోక్", role_en: "Organising Trustee", role_te: "ఆర్గనైజింగ్ ట్రస్టీ", img: "/images/organising_trustee.jpg" },
  { name_en: "Burla Venkateshwarlu", name_te: "బూర్ల వెంకటేశ్వర్లు", role_en: "Managing Trustee", role_te: "మేనేజింగ్ ట్రస్టీ", img: "/images/managing_trustee.jpg" },
  { name_en: "Devasani Srinivas", name_te: "దేవసాని శ్రీనివాస్", role_en: "Vice Chairman", role_te: "వైస్ చైర్మన్", img: "/images/vice_chairman1.jpg" },
  { name_en: "Namani Suvarna", name_te: "నామని సువర్ణ", role_en: "Vice Chairman", role_te: "వైస్ చైర్మన్", img: "/images/vice_chairman2.jpg" },
  { name_en: "V. Laxminarayana", name_te: "వి. లక్ష్మీనారాయణ", role_en: "Chief Advisor", role_te: "ముఖ్య సలహాదారు", img: "/images/chief_advisor.jpg" },
  { name_en: "Baireddy Rajireddy", name_te: "బైరెడ్డి రాజిరెడ్డి", role_en: "Co-Chief Advisor", role_te: "కో-చీఫ్ అడ్వైజర్", img: "/images/co_advisor.jpg" },
];

// --- DATA: EVENTS TIMELINE (Complete List 2013-2024) ---
const ALL_EVENTS = [
  {
    id: 101,
    year: "2024",
...
const NavBar = ({ lang, setLang, activePage, setActivePage, isMobileMenuOpen, setIsMobileMenuOpen }) => {
  const navItems = [
    ...
    <header className="sticky top-0 z-50 bg-white shadow-md border-b-4 border-amber-500">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center">
        <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setActivePage('home')}>
          ...
          />
          <div className="w-12 h-12 bg-blue-900 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg border-2 border-amber-400" style={{display: 'none'}}>
            NN
          </div>
          <div>
            ...
            <p className="text-xs text-amber-600 font-bold hidden md:block tracking-widest uppercase">
              {lang === 'en' ? "Service is our Passion" : "సేవ మా ఊపిరి"}
            </p>
          </div>
        </div>
...
const EventDetail = ({ event, lang, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-2xl w-full max-w-4xl overflow-hidden shadow-2xl relative animate-fadeIn">
        <button onClick={onClose} className="absolute top-4 right-4 bg-gray-100 hover:bg-gray-200 p-2 rounded-full z-10">
          <X className="w-6 h-6 text-gray-600" />
        </button>
        <div className="h-48 bg-gradient-to-r from-blue-900 to-blue-800 p-8 flex items-end">
           <div>
             <span className="bg-amber-500 text-white px-3 py-1 rounded-md text-sm font-bold mb-2 inline-block">
               {event.year}
             </span>
             <h2 className="text-2xl md:text-3xl font-bold text-white">{lang === 'en' ? event.title_en : event.title_te}</h2>
             <p className="text-blue-200 mt-2 flex items-center text-sm"><Calendar className="w-4 h-4 mr-2" /> {event.date}</p>
           </div>
        </div>
        <div className="p-8">
          <p className="text-gray-700 text-lg leading-relaxed mb-8">{lang === 'en' ? event.desc_en : event.desc_te}</p>
          <h3 className="text-xl font-bold text-gray-800 mb-4 border-b pb-2 flex items-center">
            <Camera className="w-5 h-5 mr-2 text-amber-500" /> {lang === 'en' ? "Gallery" : "గ్యాలరీ"}
          </h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {event.images && event.images.length > 0 ? (
                ...
                <div className="col-span-3 text-center py-8 bg-gray-50 rounded border border-dashed border-gray-300 text-gray-400">
                    <Camera className="w-12 h-12 mx-auto mb-2 opacity-50" />
                    {lang === 'en' ? "No photos added yet." : "ఫోటోలు ఇంకా జోడించబడలేదు."}
                </div>
            )}
          </div>
        </div>
...
const TimelinePage = ({ lang }) => {
  ...
  const filteredEvents = filterYear === 'All' ? ALL_EVENTS : ALL_EVENTS.filter(e => e.year === filterYear);
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          {years.map(year => (
            <button key={year} onClick={() => setFilterYear(year)}
              className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${filterYear === year ? 'bg-amber-500 text-white' : 'bg-white text-gray-600 border'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredEvents.map((event) => (
            <div key={event.id} onClick={() => setSelectedEvent(event)}
              className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all cursor-pointer group border border-gray-100 flex flex-col h-full">
              <div className="h-48 bg-gray-200 relative overflow-hidden flex items-center justify-center">
                 {event.images && event.images.length > 0 ? (
                     <img src={event.images[0]} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" onError={(e) => { e.target.style.display = 'none'; }} />
                 ) : (
                     <Camera className="w-12 h-12 text-gray-400 opacity-50" />
                 )}
                 <div className="absolute top-4 right-4 bg-amber-500 text-white px-3 py-1 rounded-md text-xs font-bold shadow-sm">{event.year}</div>
              </div>
              <div className="p-6 flex-grow flex flex-col">
                <div className="text-xs text-gray-500 font-semibold mb-2 flex items-center"><Calendar className="w-3 h-3 mr-1" /> {event.date}</div>
                <h3 className="text-lg font-bold text-gray-800 mb-3 group-hover:text-blue-700">{lang === 'en' ? event.title_en : event.title_te}</h3>
                <p className="text-gray-600 text-sm line-clamp-3">{lang === 'en' ? event.desc_en : event.desc_te}</p>
                <div className="mt-auto pt-4 border-t border-gray-100 flex items-center text-blue-600 text-sm font-bold">
                   {lang === 'en' ? "View Details" : "వివరాలు"} <ChevronRight className="w-4 h-4 ml-1" />
                </div>
              </div>
            </div>
          ))}
        </div>
        {selectedEvent && <EventDetail event={selectedEvent} lang={lang} onClose={() => setSelectedEvent(null)} />}
      </div>
    </div>
  );
};
...
// --- PAGES ---
const HomeHero = ({ lang, setActivePage }) => (
  <section className="relative bg-white text-gray-800 py-20 text-center">
    <div className="container mx-auto px-4">
      ...
      <h2 className="text-4xl md:text-6xl font-extrabold mb-6 text-blue-900">{lang === 'en' ? TRUST_INFO.name_en : TRUST_INFO.name_te}</h2>
      <p className="text-xl md:text-2xl max-w-3xl mx-auto mb-10 text-gray-600 font-serif italic">"{lang === 'en' ? TRUST_INFO.tagline_en : TRUST_INFO.tagline_te}"</p>
      <div className="flex flex-wrap justify-center gap-4">
        <button onClick={() => setActivePage('donate')} className="px-8 py-3 bg-amber-500 hover:bg-amber-600 text-white rounded-full shadow-lg text-lg font-bold flex items-center">
            <Heart className="w-5 h-5 mr-2 fill-current" /> {lang === 'en' ? "Donate Now" : "విరాళం ఇవ్వండి"}
        </button>
        <button onClick={() => setActivePage('timeline')} className="px-8 py-3 bg-white border-2 border-blue-600 text-blue-600 hover:bg-blue-50 rounded-full shadow-lg text-lg font-bold flex items-center">
            <Calendar className="w-5 h-5 mr-2" /> {lang === 'en' ? "Activities" : "కార్యక్రమాలు"}
        </button>
      </div>
    </div>
  </section>
...
const TrusteesPage = ({ lang }) => (
  <div className="min-h-screen bg-white py-12">
    <div className="container mx-auto px-4">
      <h2 className="text-3xl font-bold text-gray-800 mb-12 text-center">{lang === 'en' ? "Trust Committee" : "ట్రస్ట్ కార్యవర్గం"}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {HEADS.map((head, idx) => (
          <div key={idx} className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-2xl transition-all text-center pb-6 group">
            <div className="h-24 bg-blue-900 mb-[-40px]"></div>
            <div className="w-24 h-24 bg-white p-1 rounded-full shadow-md mx-auto relative z-10 flex items-center justify-center overflow-hidden">
                {head.img ? (
                  <img src={head.img} alt={head.name_en} className="w-full h-full object-cover rounded-full" onError={(e) => {e.target.style.display='none'; e.target.nextSibling.style.display='block'}} />
                ) : null}
                <User className="w-12 h-12 text-gray-400" style={{display: head.img ? 'none' : 'block'}} />
            </div>
            <div className="mt-4 px-4">
               <h4 className="text-lg font-bold text-gray-800 mb-1">{lang === 'en' ? head.name_en : head.name_te}</h4>
               <span className="inline-block px-3 py-1 bg-amber-100 text-amber-800 text-xs font-bold rounded-full uppercase">{lang === 'en' ? head.role_en : head.role_te}</span>
            </div>
          </div>
        ))}
      </div>
  ...
  <div className="min-h-screen bg-gray-50 py-12 flex items-center justify-center">
    <div className="container mx-auto px-4 max-w-4xl">
      <div className="bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row">
        <div className="w-full md:w-5/12 bg-blue-900 p-10 text-white flex flex-col justify-center text-center">
             ...
             <p className="text-blue-100 mb-8">{lang === 'en' ? "Your contribution helps us maintain libraries and run medical camps." : "మీ చిన్న సహాయం గ్రంథాలయాల నిర్వహణకు, వైద్య శిబిరాలకు అండగా నిలుస్తుంది."}</p>
        </div>
        <div className="w-full md:w-7/12 p-10">
            <h5 className="text-2xl font-bold text-gray-800 mb-8 border-b pb-4">{lang === 'en' ? "Bank Account Details" : "బ్యాంకు ఖాతా వివరాలు"}</h5>
            <div className="space-y-4">
                <div className="p-4 bg-gray-50 rounded border"><span className="text-xs text-gray-500 font-bold block">Account Name</span><span className="font-bold text-lg text-gray-900">NAA NESTHAM CHARITABLE TRUST</span></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 bg-gray-50 rounded border"><span className="text-xs text-gray-500 font-bold block">Account Number</span><span className="font-mono font-bold text-lg text-blue-700">62321599315</span></div>
                    <div className="p-4 bg-gray-50 rounded border"><span className="text-xs text-gray-500 font-bold block">IFSC Code</span><span className="font-mono font-bold text-lg text-gray-900">SBIN0020893</span></div>
                </div>
                <div className="p-4 bg-gray-50 rounded border"><span className="text-xs text-gray-500 font-bold block">Bank & Branch</span><span className="font-semibold text-gray-900">SBI, Ashok Nagar Branch, Karimnagar</span></div>
            </div>
        </div>
      </div>
    </div>
...
const App = () => {
  ...
  return (
    <div className="font-sans text-gray-800 min-h-screen flex flex-col bg-white">
      <NavBar lang={lang} setLang={setLang} activePage={activePage} setActivePage={setActivePage} isMobileMenuOpen={isMobileMenuOpen} setIsMobileMenuOpen={setIsMobileMenuOpen} />
      <main className="flex-grow">
        {activePage === 'home' && <HomeHero lang={lang} setActivePage={setActivePage} />}
        {activePage === 'about' && <AboutPage lang={lang} />}
        {activePage === 'timeline' && <TimelinePage lang={lang} />}
        {activePage === 'services' && <ServicesPage lang={lang} />}
        {activePage === 'trustees' && <TrusteesPage lang={lang} />}
        {activePage === 'donate' && <DonatePage lang={lang} />}
        {activePage === 'contact' && <ContactPage lang={lang} />}
      </main>
        ...
        <div className="container mx-auto px-4 text-center md:text-left">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
             ...
             <div>
                ...
                <p className="text-sm">H.No: 10-4-28/2, Bank Colony,<br/>Karimnagar, Telangana.</p>
             </div>
             <div>
                <h4 className="text-white font-bold text-lg mb-4">{lang === 'en' ? "Contact" : "ఫోన్"}</h4>
                <p className="text-sm font-mono">+91 92469 34929</p>
                <p className="text-sm font-mono">+91 94925 70745</p>
             </div>
             <div>
          ...
          <div className="border-t border-gray-800 pt-8 flex flex-col md:flex-row justify-between items-center">
            ...
            <p className="text-xs">Website built by <span className="text-amber-500 font-semibold">{TRUST_INFO.builder}</span></p>
          </div>
        </div>
      </footer>
    </div>
